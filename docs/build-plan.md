...
Ядро завершено в объёме S0–S3 и работает на devnet. S4 (mainnet-деплой, reproducible build, blackhole, бюджет циклов) отложен до решения о выходе в прод.

Единственное изменение ядра за пределами S0–S3 — шаг 6 карты проекта: `factories[]` в конфиге и атрибуция эскроу-сеттлментов донору через крейт `crown-derive` (core-spec §4). Ядро знает о шейпах ровно конвенцию заголовка (`donor` на 8..40, `salt` на 40..72, factory-spec §2.1); новый шейп = строка в `factories[]`, `REDUCE_VERSION` и `.did` при этом не меняются. Несколько `Settled` из одной транзакции засчитываются каждый.
...
**Выход.** Anchor-программа. Одна инструкция `donate(gross)`. Один CPI `transfer_checked` из ATA донора в ATA получателя — целиком. Событие через event-CPI (не `msg!`). Минт USDC — константа программы. Деплой на devnet, затем `set-upgrade-authority --final`.

**DoD.**
- `out == in`: получатель получает ровно `gross`, донор теряет ровно `gross`, тест на границах и на серии значений.
...
- **донор структурен:** попытка указать чужой ATA как источник без его подписи проваливается.
...
- E2e на devnet: донат → таймер → `get_reputation` показывает `+gross` паре `(solana-devnet, donor, recipient)`.

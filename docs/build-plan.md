...
**Дополнение от 2026-07-12:** конвенция заголовка эскроу (crown-factory, factory-spec §2.1): парсер атрибуции выучил второе смещение `salt` (40..72 — конвенция; 120..152 — two-outcome, частный случай) — последняя shape-осведомлённость ядра, дальше новый шейп = строка в `factories[]`. В testnet-профиль запиннены фабрики `payout-table` и `stream`; несколько `Settled` из одной транзакции засчитываются каждый (фикстуры — реальные devnet-транзакции). `REDUCE_VERSION` не менялся, `.did` не менялся, e2e обоих шейпов зелёный: книга показывает донору +кусок у каждого получателя, аномалий ноль. Попутно починен `dfx.json`, битый со времён среза EVM.
